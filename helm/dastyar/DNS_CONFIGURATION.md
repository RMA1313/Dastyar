# DNS Configuration for Dastyar Helm Chart

This feature allows you to configure custom DNS settings for Dastyar pods, enabling traffic redirection to proxy servers or custom endpoints.
...
Deploy:
```bash
helm upgrade --install dastyar ./helm/dastyar -f values.yaml
```
...
```bash
helm install dastyar ./helm/dastyar \
  --set dnsPolicy="None" \
...
✅ **Integration Test**
- Works with existing Dastyar configuration
...
- Kubernetes 1.19+
- Compatible with all Dastyar deployment modes
